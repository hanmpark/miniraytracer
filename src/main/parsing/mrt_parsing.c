...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/03/04 09:10:13 by hanmpark          #+#    #+#             */
/*   Updated: 2024/03/07 09:19:33 by hanmpark         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "mrt_parsing.h"

static bool	verify_scene_objs(char **lines, int *nb_objs)
{
	t_count_caps	nb_cap;
	int				i;
	...
	*nb_objs = 0;
	...
	if (nb_cap.a != 1 || nb_cap.c != 1 || nb_cap.l != 1)
...
bool	mrt_parsing(t_mrt *v, char *rt)
{
	...
	if (!rt_scene)
		return (false);
	if (verify_scene_objs(rt_scene, &v->nb_objs) == false)
