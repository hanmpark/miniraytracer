...
/*   Updated: 2024/03/07 10:00:33 by hanmpark         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static bool	dispatch_obj(t_mrt *v, char **rt_obj, int *j)
{
	...
	while (++i < ELEMENTS_MAX)
		if (!c_strcmp(*rt_obj, obj_spc[i].id))
			return (obj_spc[i].ft(v, rt_obj, j));
...
bool	read_rt_scene(t_mrt *v, char **rt)
{
	...
	while (rt[++i])
	{
		if (*rt[i] == '#')
			...
			|| !dispatch_obj(v, c_split_func(rt[i], &c_isspace), &obj_index))
		{
			error_putline(rt[i]);
